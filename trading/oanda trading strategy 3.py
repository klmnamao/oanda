...
#        header = {"Content-Type":"application/json","Authorization":"Bearer 67a64d6bf0a17b287b6f9c18890a3c78-58e4602edc9ed11420a52811ac0f456e"}
        ...
        temp['decision'] = temp['dif']-temp['difema']
        temp['result'] = temp['decision']*temp['decision'].shift(1)
        if (temp.iloc[-1]['result']<0 and temp.iloc[-1]['decision']>0 and temp.iloc[-1]['dif']<0):
            if counter == 0:
                buy(500)
                counter = 1
            elif counter == -1:
                ...
                counter = 1
            print('buy')
        elif (temp.iloc[-1]['result']<0 and temp.iloc[-1]['decision']<0 and temp.iloc[-1]['dif']>0):
            if counter == 0:
                buy(-500)
                counter = -1
...
#while datetime.now().strftime("%H:%M:%S" ) !=  '12:50:00':
    #continue'''
init_datetime = datetime(2017, 6, 27, 17, 20, 0, 0)
df = pd.DataFrame()
counter=0
date = datetime.now().strftime('%y_%m_%d_')
while True:
    #try:
        counter,temp = decision(3,39,3,counter)
        print(counter)
        temp = temp.iloc[-1]
